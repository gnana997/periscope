...
function KvMapInput({ descriptor, value, onChange, readOnly }: InputProps) {
  const obj =
    value && typeof value === "object" && !Array.isArray(value)
      ...
      : {};
  const entries = Object.keys(obj).map((k) => [k, obj[k]] as [string, unknown]);
  ...
  return (
    <div className="space-y-1.5">
        ...
        {entries.map(([k, v], i) => (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              className="w-1/3 rounded-sm border border-border bg-bg px-2.5 py-1 font-mono text-[12.5px] text-ink focus:border-accent focus:outline-none read-only:opacity-60"
              placeholder="key"
            />
            <input
      ...
      {readOnly ? null : (
        <button
          type="button"
          onClick={() => setEntries([...entries, ["", valueType === "string" ? "" : 0]])}
