...
export function ServicesPage({ cluster }: { cluster: string }) {
  ...
  const all = useMemo<Service[]>(() => (query.data as ServiceList | undefined)?.services ?? [], [query.data]);
    ...
    selectedNs && selectedName ? (
      <DetailPane
        ...
        tabs={[
          ...
          { id: "yaml", label: "yaml", ready: true, content: <YamlView cluster={cluster} source={{ kind: "builtin", yamlKind: "services" }} ns={selectedNs} name={selectedName} />, dirty: editFlag.dirty },
          { id: "events", label: "events", ready: true, content: <EventsView cluster={cluster} kind="services" ns={selectedNs} name={selectedName} /> },
        ]}
        actions={
          <ResourceActions
            cluster={cluster}
            source={{ kind: "builtin", yamlKind: "services" }}
      ...
      <SplitPane
        ...
        left={
          query.isLoading ? (
            ...
            isForbidden(query.error) ? <ForbiddenState resource="services" /> : isForbidden(query.error) ? <ForbiddenState resource="services" /> : <ErrorState title="couldn't reach the cluster" message={(query.error as Error).message} />
          ) : filtered.length === 0 ? (
