...
export function HorizontalPodAutoscalersPage({ cluster }: { cluster: string }) {
  ...
  const all = useMemo<HPA[]>(() => (query.data as HPAList | undefined)?.hpas ?? [], [query.data]);
    ...
    selectedNs && selectedName ? (
      <DetailPane
        ...
        tabs={[
          ...
          { id: "yaml", label: "yaml", ready: true, content: <YamlView cluster={cluster} source={{ kind: "builtin", yamlKind: "horizontalpodautoscalers" }} ns={selectedNs} name={selectedName} />, dirty: editFlag.dirty },
