...
import type { LimitRange, LimitRangeList } from "../lib/types";
...
import { LimitRangeDescribe } from "../components/detail/describe/LimitRangeDescribe";
...
export function LimitRangesPage({ cluster }: { cluster: string }) {
  ...
  const query = useResource({ cluster, resource: "limitranges", namespace: namespace ?? undefined });
  const all = useMemo<LimitRange[]>(() => (query.data as LimitRangeList | undefined)?.limitRanges ?? [], [query.data]);
  ...
  const columns: Column<LimitRange>[] = [
    ...
    { key: "limitCount", header: "limits", weight: 0.8, align: "right", cellClassName: "font-mono text-ink-muted", accessor: (r) => String(r.limitCount) },
  ...
  const editFlag = useEditorDirty(cluster, "limitranges", selectedNs ?? undefined, selectedName);
  ...
  const detail =
    selectedNs && selectedName ? (
      <DetailPane
        ...
        tabs={[
          { id: "describe", label: "describe", ready: true, content: <LimitRangeDescribe cluster={cluster} ns={selectedNs} name={selectedName} /> },
          { id: "yaml", label: "yaml", ready: true, content: <YamlView cluster={cluster} source={{ kind: "builtin", yamlKind: "limitranges" }} ns={selectedNs} name={selectedName} />, dirty: editFlag.dirty },
          { id: "events", label: "events", ready: true, content: <EventsView cluster={cluster} kind="limitranges" ns={selectedNs} name={selectedName} /> },
        ]}
        actions={
          <ResourceActions
            cluster={cluster}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <PageHeader
        title="LimitRanges"
        subtitle={query.isSuccess ? `${all.length} ${all.length === 1 ? "limitrange" : "limitranges"}${namespace ? ` in ${namespace}` : ""}` : undefined}
