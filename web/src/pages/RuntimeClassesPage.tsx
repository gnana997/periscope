...
import type { RuntimeClass, RuntimeClassList } from "../lib/types";
...
export function RuntimeClassesPage({ cluster }: { cluster: string }) {
  ...
  const columns: Column<RuntimeClass>[] = [
    ...
    { key: "handler", header: "handler", weight: 2, cellClassName: "font-mono text-ink-muted", accessor: (r) => r.handler },
    { key: "cpuOverhead", header: "cpu overhead", weight: 1, cellClassName: "font-mono text-ink-muted", accessor: (r) => r.cpuOverhead || "—" },
    { key: "memOverhead", header: "mem overhead", weight: 1, cellClassName: "font-mono text-ink-muted", accessor: (r) => r.memoryOverhead || "—" },
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <PageHeader
        ...
        subtitle={query.isSuccess ? `${all.length} ${all.length === 1 ? "runtimeclass" : "runtimeclasses"}` : undefined}
      />
